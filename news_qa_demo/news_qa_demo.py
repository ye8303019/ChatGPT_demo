...
embedding_file_name = "news_export_20230329110411_embedding.csv"
...
def answer_query_with_context(query: str, df: pd.DataFrame, content_embeddings: dict[(str, str), np.array],
                              show_prompt: bool = False) -> str:
    final_prompt = construct_prompt(query, content_embeddings, df)
    if show_prompt:
        print(final_prompt)
...
my_df = pd.read_csv(token_file_name)
my_df = my_df.set_index(file_index)
print(f"{len(my_df)} rows in the data.")
my_document_embeddings = load_embeddings(embedding_file_name)
...
while True:
    ...
    try:
        ...
        # 完全多轮，可以实现有记忆的问答
        result = answer_query_with_context(input_str, my_df, my_document_embeddings, True)
        print(result)
        count += 1
    except:
        print("输入错误，请重新输入。")
