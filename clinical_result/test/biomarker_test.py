import pandas as pd
from clinical_result import biomarker
...
def test():
    ...
    df = pd.read_csv('../data/biomarker_20230322.csv')
    y_true = df["manual_result"]
    y_pred = []
    for i, content in enumerate(df['content']):
        dicts = biomarker.get_final_result(content)
        values = ','.join([d['Biomarker'] for d in dicts])
    ...
    fu.write_file("biomarker_" + date_str, "test_report", report)
    return report
